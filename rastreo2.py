...
def buscarTemperaturaFueraDeRango(temperatura, listapc__):
...
#puntos consecutivos, Cuando se analice penultimo y ultimo punto de punto de la lista esto podria ocasionar un nullpointer ex
def caso0(lineas_, listapc_,x_,i_):
    if i_ <= (len(lineas_[x_].temperatura)-2):
        buscarTemperaturaFueraDeRango(lineas_[x_].temperatura[i_+1],listapc_)
        buscarTemperaturaFueraDeRango(lineas_[x_].temperatura[i_+2], listapc_)  

#1. Centro: 5, 8, 11, 14, 17, 20; arriba, abajo, izquierda, derecha. (+1,-1,-3, +3)
def caso1(lineas_, listapc_, x_, i_):
    buscarTemperaturaFueraDeRango(lineas_[x_+1].temperatura[i_], listapc_) 
    buscarTemperaturaFueraDeRango(lineas_[x_-1].temperatura[i_], listapc_) 
    buscarTemperaturaFueraDeRango(lineas_[x_-3].temperatura[i_], listapc_) 
    buscarTemperaturaFueraDeRango(lineas_[x_+3].temperatura[i_], listapc_) 
...
def caso2(lineas_, listapc_, x_, i_):
    ...
    buscarTemperaturaFueraDeRango(lineas_[x_-1].temperatura[i_], listapc_) 
    buscarTemperaturaFueraDeRango(lineas_[x_-3].temperatura[i_], listapc_) 
    buscarTemperaturaFueraDeRango(lineas_[x_+2].temperatura[i_], listapc_) 
...
for x in range(12):
    for t in lineas[x].temperatura:
        ...
        if buscarTemperaturaFueraDeRango(t, listapc) == True:
            pc = True
            listapc.append(t)
        ...
        if len(listapc) >= 3:       
            ...
            if getGradosFueraDeRango(tp) > diferenciaMax:
                diferenciaMax = getGradosFueraDeRango(tp)
                tpm = tp
